...
window.onload = async () => {
  ...
  let pacmacro_draw = () => {
    ...
    if (window.pacmacro_ctx === undefined) return;

    // fill canvas with map drawing
    window.pacmacro_ctx.drawImage(
      window.pacmacro_img_map,
      0,
      0,
  ...
  let pacmacro_recv = (e) => {
    ...
    if (msg.command === undefined) return; // invalid message

    if (msg.command == "inform") {
      let plot = convertCoords(
        window.pacmacro_map,
        msg.coordinate.latitude,
        msg.coordinate.longitude,
      );
      const p = JSON.parse(msg.data);

      if (p.id === undefined) return; // invalid
      ...
      if (p.id == window.pacmacro_ID) {
        self_summary.innerHTML = `${p.name} (${p.id}) is ${reps(p.reps)}`;
      }
